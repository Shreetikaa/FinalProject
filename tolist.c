...
void loadTasksFromFile(Task tasks[], int *numTasks);
...
void loadTasksFromFile(Task tasks[], int* numTasks) {
    ...
    if (file == NULL) {
        printf("No tasks found. Starting with an empty list.\n");
        return;
    }

    while (fscanf(file, "%99s %d\n", tasks[*numTasks].description, &tasks[*numTasks].completed) == 2) {
        (*numTasks)++;
    }
